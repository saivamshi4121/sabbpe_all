...
const categories = [
    {
        ...
        visual: (
            <div className="relative w-full h-full flex items-center justify-center">
                <div className="w-64 h-80 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md relative transform rotate-y-12 rotate-x-6 shadow-2xl skew-y-3">
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-cyan-500/10 rounded-2xl" />
                    <div className="p-6">
                        <div className="h-4 w-12 bg-white/20 rounded mb-6" />
                        <div className="h-8 w-32 bg-white/10 rounded mb-2" />
                        <div className="h-4 w-20 bg-white/10 rounded mb-8" />

                        <div className="space-y-3">
                            <div className="h-10 w-full bg-blue-500 rounded-lg flex items-center justify-center font-bold text-white text-sm shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                                Pay ₹5,000
                            </div>
                        </div>
                    </div>
                    ...
                    <motion.div
                        ...
                        className="absolute -right-8 top-12 bg-green-500 text-black font-bold px-3 py-1 rounded-full text-xs shadow-lg"
                    >
                        Success
                    </motion.div>
                </div>
        ...
        visual: (
            <div className="relative w-full h-full flex items-center justify-center">
                <div className="relative">
                    {/* Central Node */}
                    <div className="w-20 h-20 rounded-full bg-orange-500/20 border border-orange-500 flex items-center justify-center z-10 relative">
        ...
        visual: (
            <div className="relative w-full h-full flex items-center justify-center perspective-[1000px]">
                <motion.div
                    ...
                    className="w-80 h-48 bg-gradient-to-br from-indigo-900 to-purple-900 rounded-2xl border border-white/10 relative shadow-2xl flex flex-col justify-end p-6 preserve-3d"
                >
...
export default function ServicesInteractive() {
    ...
    return (
        <section className="relative w-full bg-sabbpe-navy-dark min-h-screen py-20 px-4">
            <div className="max-w-7xl mx-auto">
                <header className="mb-32 text-center md:text-left">
                    ...
                    >
                        One Platform.<br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-sabbpe-cyan to-blue-500">
                            Infinite Possibilities.
                        </span>
                    </motion.h1>
                    <p className="text-xl text-sabbpe-text-secondary max-w-2xl leading-relaxed">
                        ...
                        <nav className="space-y-2 relative">
                            ...
                            {categories.map((cat, i) => (
                                <a
                                    ...
                                    className={`relative block pl-8 py-4 transition-all duration-300 text-lg group ${activeSection === cat.id ? 'text-white font-bold' : 'text-slate-500 hover:text-slate-300'}`}
                                >
                        ...
                        {categories.map((cat, i) => (
                            ...
                            >
                                <motion.div
                                    ...
                                    viewport={{ margin: "-20%" }}
                                    className="relative rounded-3xl bg-sabbpe-navy-light/50 border border-sabbpe-border p-8 md:p-12 overflow-hidden"
                                >
                                    {/* Background Glow */}
                                    ...
                                    <div className="grid lg:grid-cols-2 gap-12 items-center relative z-10">
                                        ...
                                        <div className="order-2 lg:order-1">
                                            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-bold text-slate-300 mb-6 uppercase tracking-wider`}>
                                                0{i + 1} / {cat.title}
                                            </div>
                                            ...
                                            <div className="grid sm:grid-cols-2 gap-6 mt-10">
                                                {cat.features.map((feat, j) => (
                                                    <div key={j} className="space-y-2">
                                                        ...
                                                        <p className="text-sm text-sabbpe-text-secondary leading-relaxed">
                                                            {feat.desc}
                                                        </p>
                                                    </div>
                                                ))}
                                            ...
                                            <div className="mt-10">
                                                <button className="flex items-center gap-2 font-bold text-white hover:gap-4 transition-all group-hover:text-sabbpe-cyan">
                                        ...
                                        {/* Visual */}
                                        <div className="order-1 lg:order-2 h-[400px] lg:h-[500px] bg-black/20 rounded-2xl border border-white/5 relative overflow-hidden">
                                            {cat.visual}
                                        </div>
                                    </div>
                                </motion.div>
