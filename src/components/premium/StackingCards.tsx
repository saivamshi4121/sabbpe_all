...
const Card = ({ 
  ...
  return (
    <motion.div
      ...
      layout
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] border transition-all duration-700 bg-card cursor-pointer shadow-2xl",
    ...
    >
      ...
      <div className="absolute left-4 md:left-10 top-4 md:top-10 z-20 flex flex-col gap-2">
        <motion.div 
          layout
          className="flex items-center gap-2 md:gap-3 bg-black/40 backdrop-blur-md border border-white/10 rounded-full pr-3 md:pr-4 pl-1 py-1"
        >
          <span className="flex h-6 w-6 md:h-8 md:w-8 items-center justify-center rounded-full bg-primary text-[9px] md:text-[10px] font-black text-white shadow-lg">
            {index + 1}
          </span>
          <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">Protocol Layer</span>
        </motion.div>
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6 xs:p-8 md:p-10 lg:p-16">
        ...
        <motion.h3 
          layout
          className="mb-4 md:mb-6 text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-black tracking-tighter text-white font-display leading-[0.9]"
        >
          {item.title}
        </motion.h3>
        ...
        >
          <p className="max-w-xl text-sm xs:text-base md:text-lg lg:text-xl text-muted-foreground/90 font-light leading-relaxed">
            {item.description}
          </p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="group/btn relative h-14 w-14 md:h-16 md:w-16 lg:h-20 lg:w-20 rounded-full bg-white flex items-center justify-center overflow-hidden shrink-0 shadow-2xl"
          >
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <div 
          className={cn(
            "flex items-center gap-2 rounded-full border border-white/10 bg-black/70 p-2 backdrop-blur-3xl shadow-[0_10px_30px_-8px_rgba(0,0,0,0.8)]",
